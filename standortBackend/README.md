...
Since the introduction of E2E encryption, **the backend never sees plaintext coordinates or notes**.
All location and marker data is encrypted in the browser before it leaves the device.

The invite code is the shared group secret. It is generated by the frontend and **never sent to the backend in plaintext**. The backend only stores and compares a SHA-256 hash of the code for group membership lookups.

| Field | Wire format (what the backend stores) | What it actually contains |
|---|---|---|
| `encryptedLocation` (location update) | `e1.<iv_base64url>.<ciphertext_base64url>` | `{"lat":52.520008,"lng":13.404954,"accuracyMeters":12.5}` |
| `encryptedLocation` (marker) | `e1.<iv_base64url>.<ciphertext_base64url>` | `{"lat":52.520008,"lng":13.404954}` |
| `encryptedNotes` (marker) | `e1.<iv_base64url>.<ciphertext_base64url>` | `"Großer Parkplatz hinter der Brücke"` |
| `inviteCodeHash` (create group) | `a3f1...` (64-char lowercase SHA-256 hex) | `XXXXXXXX-XXXXXXXX` (Crockford Base32 invite code) |

The `e1.` prefix is a version tag. The IV is 12 random bytes, the ciphertext includes the 16-byte GCM authentication tag. Decryption happens exclusively in the browser using the AES-256-GCM key derived from the invite code via HKDF-SHA-256.
...
All 44 unit tests run without any external dependencies.
...
The invite code is generated by the **frontend**. The backend only receives its SHA-256 hash.
...
```json
{
  ...
  "inviteCodeHash": "a3f1b2c4d5e6f7a8b9c0d1e2f3a4b5c6d7e8f9a0b1c2d3e4f5a6b7c8d9e0f1a2"
}
```

`inviteCodeHash` is the lowercase SHA-256 hex of the invite code (`XXXXXXXX-XXXXXXXX` format). The frontend generates the invite code with `crypto.getRandomValues` and computes the hash with `crypto.subtle.digest('SHA-256', ...)` before sending.
...
The invite code is **not** returned in the response — the frontend already has it. Copy `groupId`, `memberId`, and `memberToken`.

### 2. Join group

The frontend sends the plaintext invite code. The backend computes `SHA-256(inviteCode)` internally and looks up the group by hash.
...
```json
{
  "inviteCode": "XXXXXXXX-XXXXXXXX",
  "displayName": "Max"
}
```

Invite code format: 16 Crockford Base32 characters (`XXXXXXXX-XXXXXXXX`), uppercase, without ambiguous characters I/L/O/U.
...
`historyDurationMinutes` reflects the member's stored history-window setting; for an existing display name being reclaimed, the previously stored value is returned.
...
Coordinates are **encrypted in the browser** before sending. The backend stores the opaque ciphertext blob without being able to read it.
...
```json
{
  "encryptedLocation": "e1.dGhpcmlzYW5pdg.U29tZUVuY3J5cHRlZEJ5dGVzSGVyZQ",
  "recordedAt": "2026-05-06T10:00:00Z"
}
```

`encryptedLocation` is an AES-256-GCM ciphertext in the format `e1.<iv_base64url>.<ciphertext_base64url>`.
The plaintext it decrypts to looks like this (after decryption in the browser):

```json
{"lat": 52.520008, "lng": 13.404954, "accuracyMeters": 12.5}
```

`recordedAt` remains plaintext — it is a timestamp, not a location.

Response `204 No Content`.

Validation rules:

- `encryptedLocation` must be non-empty, max. 512 characters
- `recordedAt` must be within the last 24 h and at most 60 s in the future

The server keeps a trail of past location pings per member (`recentHistory`). On every location update the trail is pruned to the member's `historyDurationMinutes` window (see endpoint 8): any ping whose `recordedAt` is older than `now - historyDurationMinutes` is dropped. Pruning happens **only on a location update from that member** — a member who stops updating keeps their last trail until they update again. A hard cap of 500 entries always applies as a safety net, regardless of the chosen duration. The current location is always kept.

### 4. Get group locations

Returns encrypted location blobs. **Decryption happens in the browser**, not in this API.
  ...
  "members": [
    {
      ...
      "recentHistory": [
        {
          "encryptedLocation": "e1.aW5pdmVjdG9yMTI.Y2lwaGVydGV4dGJ5dGVzaGVyZQ",
          "recordedAt": "2026-05-06T09:58:00Z"
        },
        {
          "encryptedLocation": "e1.bm9uY2VieXRlczEy.bW9yZWVuY3J5cHRlZGRhdGE",
          "recordedAt": "2026-05-06T09:56:00Z"
        }
      ]
    }
  ]
...
After decryption in the browser each `encryptedLocation` resolves to:
...
Would look UNENCRIPTED like this:
```json
{
  "groupId": "ad4541df-ea08-423a-b464-8567cbbde86e",
  "version": 5,
...
Coordinates and notes are **encrypted in the browser**. The marker name is stored as plaintext.
...
```json
{
  ...
  "encryptedLocation": "e1.dGhpcmlzYW5pdg.U29tZUVuY3J5cHRlZEJ5dGVzSGVyZQ",
  "encryptedNotes": "e1.bm90ZXNpdmVjdG9y.bm90ZXNjaXBoZXJ0ZXh0aGVyZQ",
  "color": "#FF8800",
  "icon": "default"
}
```

`encryptedLocation` decrypts to `{"lat":52.520008,"lng":13.404954}`.
`encryptedNotes` decrypts to `"Großer Parkplatz hinter der Brücke"`.
Both `encryptedNotes` and `color` and `icon` are optional and may be `null`.
...
- `encryptedLocation` must be non-empty, max. 512 characters
- `encryptedNotes` max. 1024 characters (optional)
- `color` max. 50 characters (optional)
- `icon` one of `"tree"`, `"book"`, `"champagne"`, `"default"`, or `null`
...
Returns all markers belonging to a group as encrypted blobs. No authentication required.
...
After decryption in the browser each `encryptedLocation` resolves to `{"lat":52.521234,"lng":13.408765}`.
...
Sets the member's history-window preference (`historyDurationMinutes`). The value is stored but existing history is **not** pruned immediately — pruning is applied on the member's next location update (endpoint 3).
...
Clears the member's stored trail (`recentHistory`) **and** their `currentLocation`. Use this to fully stop sharing: the member disappears from the map until they send a new location update. Auto-share is a client-side concept — the frontend additionally switches its auto-share toggle off when this button is used.
...
- No auth on `GET /locations` and `GET /markers` in V1: anyone who knows the `groupId` can read the encrypted blobs. Without the invite code they cannot decrypt them. A read-token will be added in V2 if needed.
...
- Invite code format: 16 Crockford Base32 characters in `XXXXXXXX-XXXXXXXX` form (~80 bit entropy), without ambiguous characters I/L/O/U. The code is generated by the frontend and never stored by the backend in plaintext.
