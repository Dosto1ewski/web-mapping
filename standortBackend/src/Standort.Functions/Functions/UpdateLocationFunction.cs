using FluentValidation;
using Microsoft.AspNetCore.Http;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Azure.Functions.Worker;
using Microsoft.Net.Http.Headers;
using Standort.Application.DTOs;
using Standort.Application.Services;
using Standort.Domain.DomainExceptions;
using Standort.Functions.Http;

namespace Standort.Functions.Functions;

public sealed class UpdateLocationFunction
{
    private readonly LocationService _locationService;
    private readonly IValidator<UpdateLocationRequest> _validator;

    public UpdateLocationFunction(LocationService locationService, IValidator<UpdateLocationRequest> validator)
    {
        _locationService = locationService;
        _validator = validator;
    }

    [Function("UpdateLocation")]
    public async Task<IActionResult> Run(
        [HttpTrigger(AuthorizationLevel.Anonymous, "put",
            Route = "groups/{groupId}/members/{memberId}/location")]
        HttpRequest req,
        string groupId,
        string memberId,
        CancellationToken ct)
    {
        var token = ExtractBearerToken(req);
        if (token is null)
        {
            return ErrorResponses.Unauthorized("Authorization header missing or malformed.");
        }

        UpdateLocationRequest? body;
        try
        {
            body = await req.ReadFromJsonAsync<UpdateLocationRequest>(ct);
        }
        catch (Exception ex)
        {
            return ErrorResponses.BadRequest($"Malformed JSON body: {ex.Message}");
        }
        if (body is null)
        {
            return ErrorResponses.BadRequest("Request body is required.");
        }

        var validation = await _validator.ValidateAsync(body, ct);
        if (!validation.IsValid)
        {
            return ErrorResponses.ValidationFailed(validation);
        }

        try
        {
            await _locationService.UpdateLocationAsync(groupId, memberId, token, body, ct);
            return new NoContentResult();
        }
        catch (MemberNotFoundException)
        {
            return ErrorResponses.NotFound("Member not found.");
        }
        catch (GroupNotFoundException)
        {
            return ErrorResponses.NotFound("Group not found.");
        }
        catch (InvalidTokenException)
        {
            return ErrorResponses.Unauthorized();
        }
        catch (ConcurrencyException ex)
        {
            return ErrorResponses.Conflict(ex.Message);
        }
    }

    private static string? ExtractBearerToken(HttpRequest req)
    {
        if (!req.Headers.TryGetValue(HeaderNames.Authorization, out var values))
        {
            return null;
        }
        var raw = values.ToString();
        const string prefix = "Bearer ";
        if (!raw.StartsWith(prefix, StringComparison.OrdinalIgnoreCase))
        {
            return null;
        }
        var token = raw[prefix.Length..].Trim();
        return string.IsNullOrEmpty(token) ? null : token;
    }
}
